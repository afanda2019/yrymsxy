  ...
  export default {
    ...
    methods:{
      openPage(str,id,type){
        ...
        this.$setStorage('id','');
        this.$router.push({name:str,query: {buyType:0,type:2,"id":"0","goods_id":this.$route.query.type_id,"goods_sku_id":this.fenleiid,"goods_nums":this.num,goods_name:this.shop.goods_name,sale_price:this.shop.sale_price,image_uri:this.shop.image_uri }});
      },
      index_info(){
        this.$ajax('spikeDetail',{
        'userToken':this.$getStorage('token'),
        'spike_id':this.$route.query.type_id
        }).then(res => {
          console.log(res)
          if(res.status == 0){
            this.shop = res.data;
