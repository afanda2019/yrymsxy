    ...
    <div class="tab-content">
      <div class="classes-list">
        <van-list v-model="loading" :finished="finished"  finished-text="没有更多了" @load="onLoad" >
          <div class="classes-item"  @click='openPage("order_info",item.id)' v-for='(item,key) in list' :key="key">
            <p class='orderno'><span>{{item.create_time_ymd}}</span><van-icon style='font-size:18px' name="delete" @click.stop='del(item.id)' v-if='item.pay_status==0||item.pay_status==255' /></p>
